"""Generic Arrow IPC export container v1. PyArrow is imported only at render time."""
...
ARROW_CONTAINER_ID = "arrow"
ARROW_CONTAINER_VERSION = 1
ARROW_DATA_CARD_SCHEMA = "veriformis.arrow-data-card/v1"
ARROW_TRAIN_PATH = "data/train.arrow"
ARROW_EVALUATION_PATH = "data/evaluation.arrow"
ARROW_DATA_CARD_PATH = "metadata/dataset-card.json"
ARROW_PROVENANCE_PATH = "metadata/row-provenance.jsonl"
ARROW_README_PATH = "README.md"
...
class _StrictArrowModel(BaseModel):
...
class ArrowDataCard(_StrictArrowModel):
    schema_version: Literal["veriformis.arrow-data-card/v1"] = ARROW_DATA_CARD_SCHEMA
    container_id: Literal["arrow"] = ARROW_CONTAINER_ID
    container_version: Literal[1] = ARROW_CONTAINER_VERSION
    ...
    row_set_id: str
    split_result_id: str
    train_path: Literal["data/train.arrow"] = ARROW_TRAIN_PATH
    train_row_count: int
    evaluation_path: Literal["data/evaluation.arrow"] = ARROW_EVALUATION_PATH
    evaluation_row_count: int
    provenance_path: Literal["metadata/row-provenance.jsonl"] = ARROW_PROVENANCE_PATH
    ...
    def _closed_layout(self) -> Self:
        if self.train_row_count < 1:
            raise ValueError("arrow export requires a non-empty train partition")
            ...
            tuple(
                sorted(
                    (
                        ...
                        ARROW_DATA_CARD_PATH,
                        self.provenance_path,
                    )
                )
            ),
            label="arrow data card paths",
        )
        return self
...
def _partition_arrow_bytes(rows: Sequence[ProductRow], row_schema: str) -> bytes:
    ...
    try:
        ...
        if table.schema != expected:
            raise ExportVerificationError(
                "arrow schema differs from the pinned Arrow schema"
            )
        return tuple(table.to_pylist())
    except ExportVerificationError:
        raise
    except Exception as exc:
        raise ExportVerificationError(f"invalid arrow partition: {exc}") from exc
...
def _provenance_from_jsonl_bytes(data: bytes) -> tuple[RowProvenance, ...]:
    if not data.endswith(b"\n"):
        raise ExportVerificationError("arrow provenance must end with one canonical LF")
    rows: list[RowProvenance] = []
    for line in data.split(b"\n")[:-1]:
        if not line:
            raise ExportVerificationError("arrow provenance must have one object per line")
        try:
            ...
            raise ExportVerificationError(
                f"invalid arrow provenance row: {exc}"
            ) from exc
    checked = tuple(rows)
    if _provenance_jsonl(checked) != data:
        raise ExportVerificationError("arrow provenance bytes are not canonical")
    return checked


def _data_card(row_set: RowSet) -> ArrowDataCard:
    objective_ids = {item.objective_id for item in row_set.provenance}
    if len(objective_ids) != 1:
        raise ExportContractError(
            "arrow export requires one objective identity across the source row set"
        )
    return ArrowDataCard(
...
def _readme_bytes(card: ArrowDataCard) -> bytes:
    columns = ", ".join(f"`{column}`" for column in card.columns)
    text = (
        "# Veriformis Arrow export\n\n"
        "This trainer-neutral export preserves the verified dataset's semantic "
        "rows and authoritative partitions as Arrow IPC files.\n\n"
        ...
        "On-disk Arrow IPC bytes of this pinned extra are bound by the export "
...
def _sidecar_files(row_set: RowSet) -> dict[str, bytes]:
    ...
    return {
        ARROW_README_PATH: _readme_bytes(card),
        ARROW_DATA_CARD_PATH: card.canonical_bytes(),
        ARROW_PROVENANCE_PATH: _provenance_jsonl(row_set.provenance),
    }


def _semantic_contents(row_set: RowSet) -> dict[str, bytes]:
    files = _sidecar_files(row_set)
    files[ARROW_TRAIN_PATH] = columnar_partition_preimage_bytes(
        row_schema=row_set.row_schema,  # type: ignore[arg-type]
        partition="train",
        payloads=_payloads(row_set.train_rows),
    )
    files[ARROW_EVALUATION_PATH] = columnar_partition_preimage_bytes(
        row_schema=row_set.row_schema,  # type: ignore[arg-type]
...
def _file_plans(
    ...
    if descriptor.selector != (
        ...
        raise ExportContractError("arrow descriptor selector changed")
    if row_set.row_schema not in descriptor.supported_row_schemas:
        raise ExportContractError(
            f"arrow v1 does not support source row schema {row_set.row_schema!r}"
        )
    contents = _semantic_contents(row_set)
    roles = {
        ARROW_README_PATH: "readme",
        ARROW_TRAIN_PATH: "training-partition",
        ARROW_EVALUATION_PATH: "evaluation-partition",
        ARROW_DATA_CARD_PATH: "dataset-card",
        ARROW_PROVENANCE_PATH: "row-provenance",
    }
    media_types = {
        ARROW_README_PATH: "text/markdown",
        ARROW_TRAIN_PATH: ARROW_MEDIA_TYPE,
        ARROW_EVALUATION_PATH: ARROW_MEDIA_TYPE,
        ARROW_DATA_CARD_PATH: "application/json",
        ARROW_PROVENANCE_PATH: "application/jsonl",
    }
    scopes = {
        ARROW_TRAIN_PATH: "train",
        ARROW_EVALUATION_PATH: "evaluation",
    }
    counts: dict[str, int | None] = {
        ARROW_TRAIN_PATH: row_set.train_row_count,
        ARROW_EVALUATION_PATH: row_set.evaluation_row_count,
        ARROW_PROVENANCE_PATH: row_set.total_row_count,
    }
    return tuple(
...
def _render(plan: ExportPlan, row_set: RowSet) -> _RenderedDerivative:
    if (
        plan.container_profile.container_id != ARROW_CONTAINER_ID
        or plan.container_profile.container_version != ARROW_CONTAINER_VERSION
        or plan.container_profile.determinism_claim != "semantic_content_only"
        or plan.consumer_profile is not None
    ):
        raise ExportVerificationError("arrow renderer received another profile")
    expected = _file_plans(ARROW_DESCRIPTOR, row_set)
    if plan.file_plans != expected:
        raise ExportVerificationError("arrow plan differs from the pinned file contract")
    sidecars = _sidecar_files(row_set)
    files = {
        ARROW_TRAIN_PATH: _partition_arrow_bytes(
            row_set.train_rows, row_set.row_schema
        ),
        ARROW_EVALUATION_PATH: _partition_arrow_bytes(
...
def _replay(
    ...
    by_path = dict(files)
    card = ArrowDataCard.from_json_bytes(by_path[ARROW_DATA_CARD_PATH])
    if card.row_schema not in V1_ROW_SCHEMA_KINDS:
        raise ExportVerificationError("arrow data card row schema is unsupported")
    train_payloads = _read_partition_payloads(by_path[ARROW_TRAIN_PATH], card.row_schema)
    evaluation_payloads = _read_partition_payloads(
        by_path[ARROW_EVALUATION_PATH], card.row_schema
    )
    provenance = _provenance_from_jsonl_bytes(by_path[ARROW_PROVENANCE_PATH])
    if len(train_payloads) != card.train_row_count:
        raise ExportVerificationError("arrow train count differs from its data card")
    if len(evaluation_payloads) != card.evaluation_row_count:
        raise ExportVerificationError(
            "arrow evaluation count differs from its data card"
        )
    ...
    for payload, item in zip(payloads, provenance, strict=True):
        ...
        if row.row_id != item.row_id or row.payload_sha256 != item.payload_sha256:
            raise ExportVerificationError(
                "arrow payload differs from aligned provenance"
            )
        rows.append(row)
    train_rows = tuple(rows[: card.train_row_count])
    evaluation_rows = tuple(rows[card.train_row_count :])
    semantic = {
        ARROW_TRAIN_PATH: columnar_partition_preimage_bytes(
            row_schema=card.row_schema,  # type: ignore[arg-type]
            partition="train",
            payloads=train_payloads,
        ),
        ARROW_EVALUATION_PATH: columnar_partition_preimage_bytes(
            row_schema=card.row_schema,  # type: ignore[arg-type]
            partition="evaluation",
            payloads=evaluation_payloads,
        ),
        ARROW_DATA_CARD_PATH: by_path[ARROW_DATA_CARD_PATH],
        ARROW_PROVENANCE_PATH: by_path[ARROW_PROVENANCE_PATH],
        ARROW_README_PATH: by_path[ARROW_README_PATH],
    }
    expected_readme = _readme_bytes(card)
    if semantic[ARROW_README_PATH] != expected_readme:
        raise ExportVerificationError("arrow README differs from its data card")
...
ARROW_DESCRIPTOR = ExportProfileDescriptor(
    ...
    dependencies=(
        ExportDependencyBinding.create(
            ...
            dependency_role="arrow-ipc-renderer",
        ),
    ),
    supported_row_schemas=_SUPPORTED_ROW_SCHEMAS,
)

ARROW_IMPLEMENTATION = _ExportImplementation(
...
validate_export_path_set(
    (
        ...
        ARROW_DATA_CARD_PATH,
        ARROW_PROVENANCE_PATH,
        EXPORT_RECEIPT_PATH,
    ),
    label="arrow output tree",
