...
Phase 9 columnar containers are in progress under item 9.5: Parquet v1
and Arrow IPC v1 are executable generic exports with semantic
fingerprints. Taxonomy still lists them planned. Hugging Face Dataset
refuses with item 9.6.

**Review date:** 2026-08-23 (independent-product Phase 9.5 Arrow)

**Next review:** Phase 9.5 pull-request merge, item 9.6, beta label cut,
...
| `export discover` | Lists executable verified-export implementations from the private service catalog | Canonical discovery response containing `arrow`, `constrained-csv`, `json`, `parquet`, `split-jsonl-directory` v1, and TRL / MLX-LM adapters |
