...
| `mapping-detect PATH` | Proposes mapping-plan/v1 objects for one JSONL, JSON, compatible CSV, Parquet, or Arrow file, including a confirmation digest; never writes a workspace | Runtime-only `veriformis.mapping-detect/v1` JSON; list-valued `turns` and `steps` match `map`; exit `2` when no detector matches |
...
| `export-verify --request-json JSON_TEXT` | Re-verifies source authority, re-derives the confirmed plan, and independently verifies the destination | Canonical source-bound verification response |
