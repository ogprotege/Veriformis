...
optional publication is in progress under its own packet. Item 19.6
adds retained project-spec fixtures, a committed fingerprint, and a CI
example job that does not replace golden-compile. There is no Hub
execute, no GitHub secrets, and no xcodebuild. Default `review_policy`
stays `none`. No heuristic blocks seal.

**Review date:** 2026-08-31 (independent-product Phase 19.6 CI examples)
...
| In-progress independent Phase 19.6 | Retained project-spec example and CI fingerprint job; golden-compile remains; no Hub |
...
Item 19.6 adds retained project-spec fixtures and a CI example.
Golden-compile remains. There is no Hub execute or quality-report
command. There is no `GeneratorPass`. Do not start Phase 20 from this
packet.

The next behavior change is item 19.7 publication ADR. Hub execute
