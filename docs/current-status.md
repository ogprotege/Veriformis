...
skips publication retry and idempotency because ADR-0020 Decision A
installs no execute adapter. There is no Hub execute. Default
`review_policy` stays `none`. No heuristic blocks seal.

**Review date:** 2026-08-31 (independent-product Phase 19.9 retry skip)

**Next review:** Phase 19.9 merge, item 19.10 adversarial closeout, or any
...
| In-progress independent Phase 19.9 | Publication retry skipped with a record; ADR-0020 Decision A; no Hub execute |
...
Item 19.9 skips publication retry with a record. There is no Hub execute
or quality-report command. There is no `GeneratorPass`. Do not start
Phase 20 from this packet.

The next behavior change is item 19.10 adversarial closeout.
