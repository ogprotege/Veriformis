...
| `dataset-row` | The files already contain training rows in JSONL, JSON, or compatible CSV. Mapping fills an existing catalog representation. Constructors do not run. | `veriformis parse … --mode dataset-row` then `veriformis map …` |
| `mixed` | You need both document-constructed rows and imported rows in one product later. Parse each family separately. Do not fuse a `.txt` file and a `.jsonl` file in one parse. | `--mode mixed` only when every path is already one family |

Suffix `.jsonl`, `.json`, or `.csv` does not switch paths. The same JSON array
compiled as document-source becomes IR paragraphs; compiled as dataset-row it
becomes captured objects.
