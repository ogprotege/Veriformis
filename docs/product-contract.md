...
complete as PR #122 at `ef31559c9184b553209a3c45eca5d943fbb9a680`; preview-only, no heuristic blocks seal; Phase 14 review workflows complete; Phase 15 scale work complete with empty published_tiers; Phase 16.4 declares read-only built-in capabilities without a loader or public plugin API; version `0.1.0` development alpha
...
Phases 0–19 complete with additive
...
generator under ADR-0018 Decision A; Mac workbench is a thin CLI adapter
under ADR-0019 Decision A with document-source, dataset-row, and mixed
compiler paths and confirm-then-map (`veriformis.workbench-adapter/v1` is
a schema pin, not a second catalog);
...
**Next review:** Phase 20 packet open, or any product-contract change. No
modest fig-leaf tier. No public plugin API. Do not start Phase 20 from
the Phase 19 packet.
