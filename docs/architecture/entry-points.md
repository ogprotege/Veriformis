...
The CLI exposes 35 root names (34 `@app.command` entries plus the `export`
...
`mapping-rejections`, `profile-admissions`, `preflight`, `goal-preview`,
...
```mermaid
flowchart TD
    script["console script: veriformis"] --> cli["cli.py: 35-name Typer adapter"]
