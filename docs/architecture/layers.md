...
service), `recipes/`, `goals/`, `mapping/`, `profiles/`, `handoff/`, `mcp/`,
and `cli.py` (Typer adapter). The
...
structural property, because it makes the compiler-path data flow and
...
stage graph. Phase 4.4 keeps read-only plan population in the same service. It
derives the complete source membership baseline from the verifier-returned row
set and provenance, creates an existing strict `ExportPlan`, and adds neither a
...
policy. Phase 4.7 remains in that same axial layer: the service invokes a
private renderer twice from fresh
...
a fixed request-v1 dataset/provenance tree and no options. Phase 5.3 adds the
exact-byte `constrained-csv` v1 planner and renderer in the same layer, with a
fixed request-v1 train/evaluation/data-card/provenance tree and no options. It
admits the three flat row schemas and refuses nested `messages` with a JSON
alternative. Normalized rows,
