...
The central directory admits only the ZIP64 extension generated by the
canonical writer: one tag `0x0001`, with the required uncompressed size,
compressed size, and/or local-header offset in ZIP order. Values must equal
the decoded central-directory record. Unnecessary, duplicate, malformed,
or unrelated extra fields refuse. Complete archive-byte comparison remains
required after structural checks.
