...
and inspection (`goals`, `presets`, `modes`, `mapping-contracts`, `mapping-detect`, `preflight`, `goal-preview`), local MCP (`mcp`), verified
exports (`export`, `export-verify`), and `version`. The complete root surface is
31 commands; `export` contains four subcommands.
