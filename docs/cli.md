...
and inspection (`goals`, `presets`, `modes`, `mapping-contracts`, `mapping-detect`, `mapping-preview`, `preflight`, `goal-preview`), local MCP (`mcp`), verified
exports (`export`, `export-verify`), and `version`. The complete root surface is
32 commands; `export` contains four subcommands.
