...
| D-23 | `macos/Tests/Post20HardeningTests.swift` covers concurrent drops, real process-group cancellation, quit draining, repeated replies, typed receipts, history migration, and unknown versions/fields; `CLIBridgeTests.swift` accepts new discovery identifiers and runs all 74 real CLI cells; `tests/regressions/test_command_results.py` binds split/seal/package JSON to actual output bytes; final Mac gate: 122 tests, 0 failures |
