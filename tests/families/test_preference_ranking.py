"""Phase 17.6: admit preference-and-ranking from user-provided pairs."""
...
from veriformis.construction.constructors import construct_preference_pair
...
from veriformis.families.preference import (
    PREFERENCE_FAMILY_ID,
    PREFERENCE_LOSS_POLICY,
    PREFERENCE_OBJECTIVE,
    PREFERENCE_ROW_SCHEMA,
    pair_set_id,
...
GOAL = "prefer-chosen-over-rejected"
REPRESENTATION = "prompt-chosen-rejected"
PAIRS = (("prompt", "prompt"), ("chosen", "chosen"), ("rejected", "rejected"))


def _write_row(
    path: Path, *, prompt: str, chosen: str, rejected: str
) -> None:
    path.write_text(
        json.dumps(
...
def _plan(workspace: Path) -> MappingPlan:
    ...
    return MappingPlan.create(
        ...
        row_schema=PREFERENCE_ROW_SCHEMA,
...
def _compile_two_sources(tmp_path: Path) -> dict[str, object]:
    ...
    first = tmp_path / "pair-a.jsonl"
    second = tmp_path / "pair-b.jsonl"
    _write_row(
        first,
    ...
    return {
        ...
        "pairs": (
            ...
            ("what freezes water", "cold", "heat"),
        ),
    }


def test_preference_is_implemented_with_new_schema_and_loss() -> None:
    assert PREFERENCE_FAMILY_ID in IMPLEMENTED_TRAINING_FAMILIES
    assert PREFERENCE_FAMILY_ID not in PLANNED_TRAINING_FAMILIES
    assert family_for_objective(PREFERENCE_OBJECTIVE) == PREFERENCE_FAMILY_ID
    assert loss_policy_for_row(PREFERENCE_ROW_SCHEMA) == PREFERENCE_LOSS_POLICY
    pin = preference_admission()
    assert pin.lifecycle == "admitted"
    assert pin.family_id == PREFERENCE_FAMILY_ID
    assert pin.generation_allowed is False
    assert pin.profile_eligibility == ()
    assert pin.row_schema_ids == (PREFERENCE_ROW_SCHEMA,)
    assert pin.leakage_grouping_keys == ("shared-prompt", "source")


def test_preference_maps_seals_verifies_and_replays(tmp_path: Path) -> None:
    ...
    assert pair_set_id(first["pairs"]) == pair_set_id(second["pairs"])  # type: ignore[arg-type]


def test_missing_or_empty_chosen_is_refused(tmp_path: Path) -> None:
    source = tmp_path / "rows.jsonl"
    source.write_text(
        json.dumps({"prompt": "sky", "chosen": "", "rejected": "green"}) + "\n",
        encoding="utf-8",
    )
    workspace = tmp_path / "ws"
...
def test_missing_or_empty_rejected_is_refused(tmp_path: Path) -> None:
    source = tmp_path / "rows.jsonl"
    source.write_text(
        json.dumps({"prompt": "sky", "chosen": "blue", "rejected": ""}) + "\n",
        encoding="utf-8",
    )
    workspace = tmp_path / "ws"
    SERVICE.parse([source], workspace, source_root=tmp_path, mode="dataset-row")
...
def test_shared_prompt_cannot_straddle_partitions(tmp_path: Path) -> None:
    ...
    _write_row(first, prompt="same prompt", chosen="alpha", rejected="beta")
    _write_row(second, prompt="same prompt", chosen="alpha", rejected="delta")
    workspace = tmp_path / "ws"
    SERVICE.parse(
        [first, second],
        workspace,
...
def test_document_source_constructor_refuses_invented_pairs() -> None:
...
def test_existing_profiles_refuse_preference_schema() -> None:
    for profile in ("trl", "mlx-lm", "axolotl", "llama-factory", "aptus"):
        with pytest.raises(Exception, match="preference-pair"):
            assert_profile_row_compatible(profile, PREFERENCE_ROW_SCHEMA)


def test_constrained_csv_refuses_preference_export(tmp_path: Path) -> None:
    ...
    with pytest.raises(ExportContractError, match="preference-pair"):
        ExportService().dry_run_export(
            ExportDryRunRequest(operation="dry_run", **selection)
        )


def test_split_jsonl_and_json_emit_preference_rows(tmp_path: Path) -> None:
    compiled = _compile_two_sources(tmp_path)
    ...
    for container_id in ("split-jsonl-directory", "json"):
        ...
        if container_id == "split-jsonl-directory":
            ...
            assert '"chosen"' in combined
            assert '"rejected"' in combined


def test_cli_maps_preference_goal(tmp_path: Path) -> None:
    first = tmp_path / "a.jsonl"
    second = tmp_path / "b.jsonl"
    _write_row(first, prompt="alpha prompt", chosen="A", rejected="B")
    _write_row(second, prompt="beta prompt", chosen="C", rejected="D")
    workspace = tmp_path / "ws"
    parse = RUNNER.invoke(
        app,
        [
...
def test_named_recipe_exists_but_document_source_cannot_construct() -> None:
    objective = TrainingObjective.create(PREFERENCE_OBJECTIVE)
    assert objective.kind == PREFERENCE_OBJECTIVE
