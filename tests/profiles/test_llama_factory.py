"""Phase 10.4: emit LLaMA-Factory files without importing llamafactory."""
...
def _selection(bundle: Path, *, schema_version: str) -> dict[str, object]:
    return {
        ...
        "consumer_profile_version": LLAMA_FACTORY_PROFILE_VERSION,
...
def test_llama_factory_is_discoverable_beside_generic_split_jsonl() -> None:
    profiles = {
        profile.selector: profile
        for profile in ExportService().discover_exports().profiles
    }
    assert SELECTOR in profiles
    llama = profiles[SELECTOR]
    assert llama.consumer_profile is not None
    assert llama.consumer_profile.consumer_id == LLAMA_FACTORY_CONSUMER_ID
    assert llama.supported_row_schemas == ROW_SCHEMAS
    assert llama.container_profile.determinism_claim == "portable_exact_bytes"


def test_map_llama_factory_payload_remaps_messages_and_prompt_completion() -> None:
    alpaca = map_llama_factory_payload(
        "instruction_output",
    ...
    sharegpt = map_llama_factory_payload(
        "messages",
        {
            "messages": [
    ...
    remapped = map_llama_factory_payload(
        "prompt_completion",
...
def test_llama_factory_render_maps_every_schema_without_changing_counts(
    tmp_path: Path,
) -> None:
    ...
    for row_schema in ROW_SCHEMAS:
        ...
        train = [
            json.loads(line)
            for line in files[LLAMA_FACTORY_TRAIN_PATH].decode("utf-8").splitlines()
        ]
        evaluation = [
            json.loads(line)
            for line in files[LLAMA_FACTORY_EVALUATION_PATH]
            .decode("utf-8")
            .splitlines()
            if line
        ]
        ...
        expected_train = [
            map_llama_factory_payload(row_schema, row.payload)
        ...
        assert LLAMA_FACTORY_TRAIN_DATASET in info
        assert info[LLAMA_FACTORY_TRAIN_DATASET]["file_name"] == "train.jsonl"
        card = LlamaFactoryDataCard.from_json_bytes(
            files[LLAMA_FACTORY_DATA_CARD_PATH]
        )
        meta = LlamaFactoryProfileMetadata.from_json_bytes(
            files[LLAMA_FACTORY_PROFILE_METADATA_PATH]
        )
        assert card.loss_policy == loss_policy_for_row(row_schema)
        assert meta.round_trip is False
        launch = LlamaFactorySftLaunchSidecar.from_json_bytes(
            files[LLAMA_FACTORY_LAUNCH_PATH]
        )
        assert launch.launches_training is False
        assert launch.selects_model is False
        assert launch.command_argv == ("llamafactory-cli", "train")
        assert "--train" not in launch.command_argv
        assert files[LLAMA_FACTORY_README_PATH].endswith(b"\n")
        assert b"does not launch" in files[LLAMA_FACTORY_README_PATH]


def test_llama_factory_publishes_and_verifies_the_text_fixture(
    tmp_path: Path,
) -> None:
    bundle = _materialize_bundle(tmp_path)
    ...
    assert plan.consumer_profile.consumer_id == LLAMA_FACTORY_CONSUMER_ID
    ...
    info = json.loads(tree[LLAMA_FACTORY_DATASET_INFO_PATH].decode("utf-8"))
    assert LLAMA_FACTORY_EVALUATION_DATASET in info
    train = [
        json.loads(line)
        for line in tree[LLAMA_FACTORY_TRAIN_PATH].decode("utf-8").splitlines()
    ]
    assert train and all(set(row) == {"text"} for row in train)

    changed = tmp_path / "tampered"
    shutil.copytree(destination, changed)
    (changed / LLAMA_FACTORY_TRAIN_PATH).write_bytes(
        (changed / LLAMA_FACTORY_TRAIN_PATH).read_bytes() + b"\n"
    )
    with pytest.raises(ExportVerificationError):
        service.verify_export(_verify(bundle, changed, plan.export_plan_id))


def test_llama_factory_refuses_container_options_before_source_access(
...
def test_llama_factory_emission_does_not_import_trainer_libraries() -> None:
    assert "llamafactory" not in sys.modules
    assert "torch" not in sys.modules
    map_llama_factory_payload("text", {"text": "ok"})
    assert "llamafactory" not in sys.modules
    assert "torch" not in sys.modules
