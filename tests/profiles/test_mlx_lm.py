"""Phase 8.4: emit MLX-LM LoRA files without importing mlx-lm."""
...
def _selection(bundle: Path, *, schema_version: str) -> dict[str, object]:
    return {
        ...
        "consumer_profile_version": MLX_LM_PROFILE_VERSION,
...
def test_mlx_lm_is_discoverable_beside_generic_split_jsonl() -> None:
    profiles = {
    ...
    mlx_lm = profiles[SELECTOR]
    assert mlx_lm.consumer_profile is not None
    assert mlx_lm.consumer_profile.consumer_id == MLX_LM_CONSUMER_ID
    assert mlx_lm.supported_row_schemas == ROW_SCHEMAS
    assert mlx_lm.container_profile.determinism_claim == "portable_exact_bytes"


def test_map_mlx_lm_payload_assembles_instruction_output_and_keeps_identity() -> None:
    assembled = map_mlx_lm_payload(
        "instruction_output",
        {"instruction": "Task", "input": "Context", "output": "Answer"},
    )
    assert assembled == {"completion": "Answer", "prompt": "Task\nContext"}
    text = map_mlx_lm_payload("text", {"text": "café"})
    assert text == {"text": "café"}


def test_mlx_lm_render_maps_every_schema_without_changing_counts(tmp_path: Path) -> None:
    ...
    for row_schema in ROW_SCHEMAS:
        ...
        train = [
            json.loads(line)
            for line in files[MLX_LM_TRAIN_PATH].decode("utf-8").splitlines()
        ]
        evaluation = [
            json.loads(line)
            for line in files[MLX_LM_EVALUATION_PATH].decode("utf-8").splitlines()
            if line
        ]
        assert len(train) == row_set.train_row_count
        assert len(evaluation) == row_set.evaluation_row_count
        expected_train = [
            map_mlx_lm_payload(row_schema, row.payload) for row in row_set.train_rows
        ]
        ...
        meta = MlxLmProfileMetadata.from_json_bytes(files[MLX_LM_PROFILE_METADATA_PATH])
        ...
        assert files[MLX_LM_README_PATH].endswith(b"\n")
...
def test_mlx_lm_publishes_and_verifies_the_text_fixture(tmp_path: Path) -> None:
    ...
    assert plan.consumer_profile.consumer_id == MLX_LM_CONSUMER_ID
...
def test_empty_evaluation_omits_valid_jsonl(tmp_path: Path) -> None:
    bundle = _materialize_bundle(tmp_path)
    source = _row_set_for_schema(_source_row_set(bundle), "text")
    empty = RowSet.create(
        plan_id=source.plan_id,
        serialization_plan_id=source.serialization_plan_id,
        recipe_id=source.recipe_id,
...
def test_mlx_lm_emission_does_not_import_trainer_libraries() -> None:
    assert "mlx_lm" not in sys.modules
    assert "torch" not in sys.modules
    map_mlx_lm_payload("text", {"text": "ok"})
    assert "mlx_lm" not in sys.modules
    assert "torch" not in sys.modules
